                    ...
                    if(data.hasOwnProperty('api') && data.hasOwnProperty('file'))
                        configurationManager._getApiFileContent(data.file).then((mockContent) => {
                            $.post('http://mobile.tmall.net/rocket/api/apiAddMockConfig.php',
                                {api:data.api, mock_body:JSON.stringify(mockContent), mock_script:"", comment:"THERA_"+data.api}, function(d) {
                                    if(d==1) {
                                        console.log("mock-data 添加成功  " + d)
                                        alert('添加成功!' + 'http://mobile.tmall.net/rocket/mock.html?api=' + data.api );
                                    } else {
